            ...
            list: async (user: string): Promise<BucketResponseObj> => {
                try {
                    while (this.truncated) {
                        ...
                        if (!bucket || !bucket.Contents) return { success: false, message: 'Bucket is empty or does not exist' };
                    ...
                    if (err instanceof Error) {
                        ...
                        return { success: false, message: `Failed to fetch bucket content for ${user} with error: ${err.message}` };
                    }
