...
const MySQLUserMigration: FastifyPluginAsync = async (fastify, _opts): Promise<void> => {
    ...
    fastify.get<{ Params: UserParams }>('/user/:userId/:token', async (_request, _reply) => {
        ...
        const response = await migration.user.migrate.mysqlUser(_request.params.userId);
...
export default MySQLUserMigration;